...
public class ConnectionRequestsLaOrgSteps
{
    private readonly string _baseUrl;
    public HttpResponseMessage LastResponse { get; private set; }

    public ConnectionRequestsLaOrgSteps()
    {
    ...
    public async Task SendAValidRequestForTheTotalConnectionRequests(string bearerToken, long laOrgId)
    {
    ...
    public async Task SendAValidRequestForConnectionRequestsInThePast7days(string date, long laOrgId,
        string bearerToken)
    {
        LastResponse = await HttpRequestFactory.Get(_baseUrl,
            $"report/connection-requests-past-7-days/organisation/{laOrgId}?date={date}",
            bearerToken,
            null,
            null);
    }
    ...
    {
        LastResponse = await HttpRequestFactory.Get(_baseUrl,
            $"report/connection-requests-past-7-days/organisation/{laOrgId}",
            bearerToken,
            null,
            null);
    }
    ...
    {
        LastResponse = await HttpRequestFactory.Get(_baseUrl,
            $"report/connection-requests-4-week-breakdown/organisation/{laOrgId}?date={date}",
            bearerToken,
            null,
            null);
    }
    ...
    public async Task SendAValidRequestForConnectionRequestsFourWeekBreakdownWithAMissingDateParameter(
        string bearerToken, long laOrgId)
    {
        LastResponse = await HttpRequestFactory.Get(_baseUrl,
            $"report/connection-requests-4-week-breakdown/organisation/{laOrgId}",
            bearerToken,
            null,
            null);
    }
