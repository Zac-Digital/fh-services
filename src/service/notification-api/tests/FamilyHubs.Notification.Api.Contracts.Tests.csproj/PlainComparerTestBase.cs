...
public abstract class PlainComparerTestBase<T, TPropertyType> where T : class
{
    private readonly T _sut;
    private T _other;
    private readonly Expression<Func<T, TPropertyType?>> _propertyToTest;

    protected PlainComparerTestBase(T sut, T other, Expression<Func<T, TPropertyType?>> propertyToTest)
    {
        _sut = sut;
        _other = other;
